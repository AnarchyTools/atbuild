...
//todo, support multiple toolchains
    ...
    let SwiftStaticLibsPath = "/Library/Developer/Toolchains/swift-latest.xctoolchain//usr/lib/swift_static/macosx/"
#elseif os(Linux)
    let SwiftCPath = "/usr/local/bin/swiftc"
    let SwiftBuildToolpath = "/usr/local/bin/swift-build-tool"
    let SwiftStaticLibsPath = "/usr/local/lib/swift_static/linux/"
#endif